...
export default function AddFood() {
  ...
  const handleImageUpload = async () => {
    //now it will return from this part
    if (!input.foodImage) return;

    const data = new FormData();
    data.append("file", input.foodImage);
    data.append("upload_preset", "annadaata");
    data.append("cloud_name", "dxrxy1llh");

    try {
      const response = await fetch(
        "https://api.cloudinary.com/v1_1/dxrxy1llh/image/upload",
        {
          method: "POST",
          body: data,
        }
      ...
      <div className="max-w-4xl mx-auto p-8">
        <div className="space-y-2 text-center">
          <div>
            ...
            {/* I will update it later as it is causing error I have to read the api docs again */}
                <div>
